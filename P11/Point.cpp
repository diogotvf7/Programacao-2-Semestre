...
Point Point::operator+(const Point& p) const {return Point(x_ + p.get_x(), y_ + p.get_y());}
Point& Point::operator+=(const Point& p) {x_ += p.get_x(); y_ += p.get_y(); return *this;}
...
std::ostream& operator<<(std::ostream& os, const Point& p) {os << '(' << p.get_x() << ',' << p.get_y() << ')'; return os;}